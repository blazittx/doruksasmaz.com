document.addEventListener('DOMContentLoaded', function() {
    ...
    var events = {
        2: 'Game Jam at Maltepe University',
        15: 'MAUJAM 2023',
        // ...
    };
    
    var days = document.querySelectorAll('.calendar .days .day');
    var eventDetailElement = document.getElementById('event-detail');
    
    days.forEach(function(day) {
        var dayNumber = Number(day.textContent);
        if(events[dayNumber]) {
            day.classList.add('event-day');
            day.innerHTML += '<div class="event">' + events[dayNumber] + '</div>';
            
            day.addEventListener('click', function() {
                eventDetailElement.textContent = 'Event on ' + dayNumber + ' June: ' + events[dayNumber];
            });
        }
    });
});
